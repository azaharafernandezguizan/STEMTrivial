...
(function(process) {
  ...
  (function(ts) {
    ...
    function mapIterator(iter, mapFn) {
      return {next: function() {
          ...
          return iterRes.done ? iterRes : {
            value: mapFn(iterRes.value),
            done: false
          };
        }};
    }
    ...
    ts.emptyIterator = {next: function() {
        return ({
          value: undefined,
          done: true
        });
    ...
    function deduplicateRelational(array, equalityComparer, comparer) {
      var indices = array.map(function(_, i) {
        return i;
      });
      ...
      deduplicated.sort();
      return deduplicated.map(function(i) {
        return array[i];
      });
    }
    ...
    function deduplicate(array, equalityComparer, comparer) {
      return !array ? undefined : array.length === 0 ? [] : array.length === 1 ? array.slice() : comparer ? deduplicateRelational(array, equalityComparer, comparer) : deduplicateEquality(array, equalityComparer);
    }
    ...
    function relativeComplement(arrayA, arrayB, comparer) {
      if (!arrayB || !arrayA || arrayB.length === 0 || arrayA.length === 0)
        return arrayB;
      var result = [];
      loopB: for (var offsetA = 0,
          offsetB = 0; offsetB < arrayB.length; offsetB++) {
        if (offsetB > 0) {
    ...
    var createUIStringComparer = (function() {
      ...
      function createIntlCollatorStringComparer(locale) {
        var comparer = new Intl.Collator(locale, {
          usage: "sort",
          sensitivity: "variant"
        }).compare;
        return function(a, b) {
    ...
    function compareProperties(a, b, key, comparer) {
      return a === b ? 0 : a === undefined ? -1 : b === undefined ? 1 : comparer(a[key], b[key]);
    }
    ...
    function compareDiagnostics(d1, d2) {
      return compareStringsCaseSensitive(getDiagnosticFileName(d1), getDiagnosticFileName(d2)) || compareValues(d1.start, d2.start) || compareValues(d1.length, d2.length) || compareValues(d1.code, d2.code) || compareMessageText(d1.messageText, d2.messageText) || 0;
    }
    ...
    function getAllowSyntheticDefaultImports(compilerOptions) {
      ...
      return compilerOptions.allowSyntheticDefaultImports !== undefined ? compilerOptions.allowSyntheticDefaultImports : compilerOptions.esModuleInterop ? moduleKind !== ts.ModuleKind.None && moduleKind < ts.ModuleKind.ES2015 : moduleKind === ts.ModuleKind.System;
    }
    ...
    function matchFiles(path, extensions, excludes, includes, useCaseSensitiveFileNames, currentDirectory, depth, getFileSystemEntries) {
      path = normalizePath(path);
      currentDirectory = normalizePath(currentDirectory);
      var comparer = useCaseSensitiveFileNames ? compareStringsCaseSensitive : compareStringsCaseInsensitive;
      ...
      function visitDirectory(path, absolutePath, depth) {
        ...
        for (var _i = 0,
            _b = sort(files, comparer); _i < _b.length; _i++) {
          var current = _b[_i];
          _loop_1(current);
        }
        if (depth !== undefined) {
        ...
        for (var _c = 0,
            _d = sort(directories, comparer); _c < _d.length; _c++) {
          var current = _d[_c];
          var name = combinePaths(path, current);
          var absoluteName = combinePaths(absolutePath, current);
  ...
  (function(ts) {
    ...
    function packageIdToString(_a) {
      var name = _a.name,
          subModuleName = _a.subModuleName,
          version = _a.version;
    ...
    function getTokenPosOfNode(node, sourceFile, includeJsDoc) {
      ...
      if (node.kind === 290 && node._children.length > 0) {
    ...
    function isCatchClauseVariableDeclarationOrBindingElement(declaration) {
      ...
      return node.kind === 227 && node.parent.kind === 264;
    }
    ts.isCatchClauseVariableDeclarationOrBindingElement = isCatchClauseVariableDeclarationOrBindingElement;
    function isAmbientModule(node) {
      return node && node.kind === 234 && (node.name.kind === 9 || isGlobalScopeAugmentation(node));
    }
    ...
    function isShorthandAmbientModule(node) {
      return node && node.kind === 234 && (!node.body);
    }
    function isBlockScopedContainerTopLevel(node) {
      return node.kind === 269 || node.kind === 234 || ts.isFunctionLike(node);
    }
    ...
    function getErrorSpanForArrowFunction(sourceFile, node) {
      ...
      if (node.body && node.body.kind === 208) {
    ...
    function isConstEnumDeclaration(node) {
      return node.kind === 233 && isConst(node);
    }
    ...
    function isSuperCall(n) {
      return n.kind === 182 && n.expression.kind === 97;
    }
    ts.isSuperCall = isSuperCall;
    function isImportCall(n) {
      return n.kind === 182 && n.expression.kind === 91;
    }
    ts.isImportCall = isImportCall;
    function isPrologueDirective(node) {
      return node.kind === 211 && node.expression.kind === 9;
    }
    ...
    function getJSDocCommentRanges(node, text) {
      var commentRanges = (node.kind === 147 || node.kind === 146 || node.kind === 187 || node.kind === 188 || node.kind === 186) ? ts.concatenate(ts.getTrailingCommentRanges(text, node.pos), ts.getLeadingCommentRanges(text, node.pos)) : ts.getLeadingCommentRanges(text, node.pos);
    ...
    function isPartOfTypeNode(node) {
      if (159 <= node.kind && node.kind <= 174) {
        return true;
      }
      switch (node.kind) {
        case 119:
        ...
        case 130:
          return true;
        case 105:
          return node.parent.kind !== 191;
        case 202:
          return !isExpressionWithTypeArgumentsInClassExtendsClause(node);
        case 71:
          if (node.parent.kind === 144 && node.parent.right === node) {
            node = node.parent;
          } else if (node.parent.kind === 180 && node.parent.name === node) {
            node = node.parent;
          }
          ts.Debug.assert(node.kind === 71 || node.kind === 144 || node.kind === 180, "'node' was expected to be a qualified name, identifier or property access in 'isPartOfTypeNode'.");
        case 144:
        case 180:
        case 99:
          ...
          if (159 <= parent.kind && parent.kind <= 174) {
            return true;
          }
          switch (parent.kind) {
            case 202:
    ...
    function getRestParameterElementType(node) {
      if (node && node.kind === 165) {
        return node.elementType;
      } else if (node && node.kind === 160) {
    ...
    function isVariableDeclarationInVariableStatement(node) {
      return node.parent.kind === 228 && node.parent.parent.kind === 209;
    }
    ts.isVariableDeclarationInVariableStatement = isVariableDeclarationInVariableStatement;
    function isValidESSymbolDeclaration(node) {
      return ts.isVariableDeclaration(node) ? isConst(node) && ts.isIdentifier(node.name) && isVariableDeclarationInVariableStatement(node) : ts.isPropertyDeclaration(node) ? hasReadonlyModifier(node) && hasStaticModifier(node) : ts.isPropertySignature(node) && hasReadonlyModifier(node);
    }
    ...
    function isFunctionBlock(node) {
      return node && node.kind === 208 && ts.isFunctionLike(node.parent);
    }
    ts.isFunctionBlock = isFunctionBlock;
    function isObjectLiteralMethod(node) {
      return node && node.kind === 152 && node.parent.kind === 179;
    }
    ts.isObjectLiteralMethod = isObjectLiteralMethod;
    function isObjectLiteralOrClassExpressionMethod(node) {
      return node.kind === 152 && (node.parent.kind === 179 || node.parent.kind === 200);
    }
      ...
      while (true) {
        ...
        switch (node.kind) {
          case 145:
            ...
            break;
          case 148:
            if (node.parent.kind === 147 && ts.isClassElement(node.parent.parent)) {
    ...
    function getImmediatelyInvokedFunctionExpression(func) {
      if (func.kind === 187 || func.kind === 188) {
        var prev = func;
        var parent = func.parent;
        while (parent.kind === 186) {
          prev = parent;
          parent = parent.parent;
        }
        if (parent.kind === 182 && parent.expression === prev) {
          return parent;
        }
      }
    }
    ...
    function isSuperProperty(node) {
      var kind = node.kind;
      return (kind === 180 || kind === 181) && node.expression.kind === 97;
    }
    ts.isSuperProperty = isSuperProperty;
    function isThisProperty(node) {
      var kind = node.kind;
      return (kind === 180 || kind === 181) && node.expression.kind === 99;
    }
    ...
    function nodeCanBeDecorated(node, parent, grandparent) {
      switch (node.kind) {
        case 230:
          return true;
        case 150:
          return parent.kind === 230;
        case 154:
        case 155:
        case 152:
          return node.body !== undefined && parent.kind === 230;
        case 147:
          return parent.body !== undefined && (parent.kind === 153 || parent.kind === 152 || parent.kind === 155) && grandparent.kind === 230;
      }
      return false;
    }
    ...
    function childIsDecorated(node, parent) {
      switch (node.kind) {
        case 230:
          return ts.forEach(node.members, function(m) {
            return nodeOrChildIsDecorated(m, node, parent);
          });
        case 152:
        case 155:
          return ts.forEach(node.parameters, function(p) {
    ...
    function isJSXTagName(node) {
      var parent = node.parent;
      if (parent.kind === 252 || parent.kind === 251 || parent.kind === 253) {
    ...
    function isExpressionNode(node) {
      switch (node.kind) {
        ...
        case 205:
          return true;
        case 144:
          while (node.parent.kind === 144) {
            node = node.parent;
          }
          return node.parent.kind === 163 || isJSXTagName(node);
        case 71:
          if (node.parent.kind === 163 || isJSXTagName(node)) {
            return true;
          }
        case 8:
        case 9:
    ...
    function isInExpressionContext(node) {
      ...
      switch (parent.kind) {
        ...
        case 215:
          ...
          return (forStatement.initializer === node && forStatement.initializer.kind !== 228) || forStatement.condition === node || forStatement.incrementor === node;
        case 216:
        case 217:
          var forInStatement = parent;
          return (forInStatement.initializer === node && forInStatement.initializer.kind !== 228) || forInStatement.expression === node;
        case 185:
        case 203:
    ...
    function isExternalModuleImportEqualsDeclaration(node) {
      return node.kind === 238 && node.moduleReference.kind === 249;
    }
    ...
    function isDeclarationOfFunctionOrClassExpression(s) {
      if (s.valueDeclaration && s.valueDeclaration.kind === 227) {
        var declaration = s.valueDeclaration;
        return declaration.initializer && (declaration.initializer.kind === 187 || declaration.initializer.kind === 200);
      }
      return false;
    }
    ...
    function getSpecialPropertyAssignmentKind(expr) {
      ...
      if (expr.operatorToken.kind !== 58 || expr.left.kind !== 180) {
        return 0;
      }
      var lhs = expr.left;
      if (lhs.expression.kind === 71) {
        ...
        return 4;
      } else if (lhs.expression.kind === 180) {
    ...
    function isSpecialPropertyDeclaration(expr) {
      return isInJavaScriptFile(expr) && expr.parent && expr.parent.kind === 211 && !!ts.getJSDocTypeTag(expr.parent);
    }
    ...
    function getNamespaceDeclarationNode(node) {
      ...
      if (importClause && importClause.namedBindings && importClause.namedBindings.kind === 241) {
        return importClause.namedBindings;
      }
    }
    ts.getNamespaceDeclarationNode = getNamespaceDeclarationNode;
    function isDefaultImport(node) {
      return node.kind === 239 && node.importClause && !!node.importClause.name;
    }
    ...
    function isJSDocConstructSignature(node) {
      return node.kind === 277 && node.parameters.length > 0 && node.parameters[0].name && node.parameters[0].name.escapedText === "new";
    }
    ...
    function getNestedModuleDeclaration(node) {
      return node.kind === 234 && node.body && node.body.kind === 234 && node.body;
    }
    function getJSDocCommentsAndTags(node) {
      ...
      function getJSDocCommentsAndTagsWorker(node) {
        var parent = node.parent;
        if (parent && (parent.kind === 265 || getNestedModuleDeclaration(parent))) {
        ...
        if (ts.isBinaryExpression(node) && getSpecialPropertyAssignmentKind(node) !== 0 || node.kind === 180 && node.parent && node.parent.kind === 211) {
          getJSDocCommentsAndTagsWorker(parent);
        }
        if (node.kind === 147) {
    ...
    function isDeleteTarget(node) {
      if (node.kind !== 180 && node.kind !== 181) {
        return false;
      }
      node = walkUpParenthesizedExpressions(node.parent);
      return node && node.kind === 189;
    }
    ...
    function isLiteralComputedPropertyDeclarationName(node) {
      return (node.kind === 9 || node.kind === 8) && node.parent.kind === 145 && ts.isDeclaration(node.parent.parent);
    }
      ...
      switch (parent.kind) {
        ...
        case 144:
          if (parent.right === node) {
            while (parent.kind === 144) {
              parent = parent.parent;
            }
            return parent.kind === 163;
          }
          return false;
        case 177:
        case 243:
    ...
    function isAliasSymbolDeclaration(node) {
      return node.kind === 238 || node.kind === 237 || node.kind === 240 && !!node.name || node.kind === 241 || node.kind === 243 || node.kind === 247 || node.kind === 244 && exportAssignmentIsAlias(node);
    }
    ...
    function isKeyword(token) {
      return 72 <= token && token <= 143;
    }
    ts.isKeyword = isKeyword;
    function isContextualKeyword(token) {
      return 117 <= token && token <= 143;
    }
    ...
    function isDynamicName(name) {
      return name.kind === 145 && !isStringOrNumericLiteral(name.expression) && !isWellKnownSymbolSyntactically(name.expression);
    }
    ...
    function nodeStartsNewLexicalEnvironment(node) {
      var kind = node.kind;
      return kind === 153 || kind === 187 || kind === 229 || kind === 188 || kind === 152 || kind === 154 || kind === 155 || kind === 234 || kind === 269;
    }
    ...
    function getExpressionAssociativity(expression) {
      ...
      var hasArguments = expression.kind === 183 && expression.arguments !== undefined;
    ...
    function getOperator(expression) {
      if (expression.kind === 195) {
        return expression.operatorToken.kind;
      } else if (expression.kind === 193 || expression.kind === 194) {
    ...
    function getFirstConstructorWithBody(node) {
      return ts.forEach(node.members, function(member) {
        if (member.kind === 153 && nodeIsPresent(member.body)) {
          return member;
        }
      });
    }
    ...
    function getAllAccessorDeclarations(declarations, accessor) {
      ...
      if (hasDynamicName(accessor)) {
        ...
        if (accessor.kind === 154) {
          getAccessor = accessor;
        } else if (accessor.kind === 155) {
        ...
        ts.forEach(declarations, function(member) {
          if ((member.kind === 154 || member.kind === 155) && hasModifier(member, 32) === hasModifier(accessor, 32)) {
            ...
            if (memberName === accessorName) {
              ...
              if (member.kind === 155 && !setAccessor) {
                setAccessor = member;
              }
            }
          }
    ...
    function tryGetClassExtendingExpressionWithTypeArguments(node) {
      if (node.kind === 202 && node.parent.token === 85 && ts.isClassLike(node.parent.parent)) {
        return node.parent.parent;
      }
    }
    ...
    function isExpressionWithTypeArgumentsInClassImplementsClause(node) {
      return node.kind === 202 && isEntityNameExpression(node.expression) && node.parent && node.parent.token === 108 && node.parent.parent && ts.isClassLike(node.parent.parent);
    }
    ts.isExpressionWithTypeArgumentsInClassImplementsClause = isExpressionWithTypeArgumentsInClassImplementsClause;
    function isEntityNameExpression(node) {
      return node.kind === 71 || node.kind === 180 && isEntityNameExpression(node.expression);
    }
    ts.isEntityNameExpression = isEntityNameExpression;
    function isRightSideOfQualifiedNameOrPropertyAccess(node) {
      return (node.parent.kind === 144 && node.parent.right === node) || (node.parent.kind === 180 && node.parent.name === node);
    }
    ts.isRightSideOfQualifiedNameOrPropertyAccess = isRightSideOfQualifiedNameOrPropertyAccess;
    function isEmptyObjectLiteral(expression) {
      return expression.kind === 179 && expression.properties.length === 0;
    }
    ts.isEmptyObjectLiteral = isEmptyObjectLiteral;
    function isEmptyArrayLiteral(expression) {
      return expression.kind === 178 && expression.elements.length === 0;
    }
    ...
    function accessKind(node) {
      ...
      function writeOrReadWrite() {
        return parent.parent && parent.parent.kind === 211 ? 1 : 2;
      }
    }
    ...
    function forSomeAncestorDirectory(directory, callback) {
      return !!forEachAncestorDirectory(directory, function(d) {
        return callback(d) ? true : undefined;
      });
    }
  ...
  (function(ts) {
    ...
    function getTypeParameterOwner(d) {
      if (d && d.kind === 146) {
        for (var current = d; current; current = current.parent) {
          if (ts.isFunctionLike(current) || ts.isClassLike(current) || current.kind === 231) {
            return current;
          }
        }
      }
    ...
    function isParameterPropertyDeclaration(node) {
      return ts.hasModifier(node, 92) && node.parent.kind === 153 && ts.isClassLike(node.parent.parent);
    }
    ...
    function walkUpBindingElementsAndPatterns(node) {
      while (node && (node.kind === 177 || ts.isBindingPattern(node))) {
        node = node.parent;
      }
      return node;
    }
    function getCombinedModifierFlags(node) {
      ...
      if (node && node.kind === 228) {
        flags |= ts.getModifierFlags(node);
        node = node.parent;
      }
      if (node && node.kind === 209) {
        flags |= ts.getModifierFlags(node);
      }
      return flags;
    }
    ...
    function getJSDocTypeTag(node) {
      var tag = getFirstJSDocTag(node, 286);
  ...
  (function(ts) {
    ...
    function isBreakOrContinueStatement(node) {
      return node.kind === 219 || node.kind === 218;
    }
    ...
    function isJSDocPropertyLikeTag(node) {
      return node.kind === 289 || node.kind === 284;
    }
  ...
  (function(ts) {
    ...
    function isEntityName(node) {
      var kind = node.kind;
      return kind === 144 || kind === 71;
    }
    ts.isEntityName = isEntityName;
    function isPropertyName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 9 || kind === 8 || kind === 145;
    }
    ts.isPropertyName = isPropertyName;
    function isBindingName(node) {
      var kind = node.kind;
      return kind === 71 || kind === 175 || kind === 176;
    }
    ...
    function isClassElement(node) {
      var kind = node.kind;
      return kind === 153 || kind === 150 || kind === 152 || kind === 154 || kind === 155 || kind === 158 || kind === 207 || kind === 248;
    }
    ts.isClassElement = isClassElement;
    function isClassLike(node) {
      return node && (node.kind === 230 || node.kind === 200);
    }
    ts.isClassLike = isClassLike;
    function isAccessor(node) {
      return node && (node.kind === 154 || node.kind === 155);
    }
    ...
    function isTypeElement(node) {
      var kind = node.kind;
      return kind === 157 || kind === 156 || kind === 149 || kind === 151 || kind === 158 || kind === 248;
    }
    ts.isTypeElement = isTypeElement;
    function isObjectLiteralElementLike(node) {
      var kind = node.kind;
      return kind === 265 || kind === 266 || kind === 267 || kind === 152 || kind === 154 || kind === 155 || kind === 248;
    }
    ts.isObjectLiteralElementLike = isObjectLiteralElementLike;
    function isTypeNodeKind(kind) {
      return (kind >= 159 && kind <= 174) || kind === 119 || kind === 133 || kind === 134 || kind === 122 || kind === 136 || kind === 137 || kind === 99 || kind === 105 || kind === 139 || kind === 95 || kind === 130 || kind === 202 || kind === 272 || kind === 273 || kind === 274 || kind === 275 || kind === 276 || kind === 277 || kind === 278;
    }
    ...
    function isCallOrNewExpression(node) {
      return node.kind === 182 || node.kind === 183;
    }
    ts.isCallOrNewExpression = isCallOrNewExpression;
    function isTemplateLiteral(node) {
      var kind = node.kind;
      return kind === 197 || kind === 13;
    }
    ...
    function isForInOrOfStatement(node) {
      return node.kind === 216 || node.kind === 217;
    }
    ...
    function isModuleBody(node) {
      var kind = node.kind;
      return kind === 235 || kind === 234 || kind === 71;
    }
    ts.isModuleBody = isModuleBody;
    function isNamespaceBody(node) {
      var kind = node.kind;
      return kind === 235 || kind === 234;
    }
    ts.isNamespaceBody = isNamespaceBody;
    function isJSDocNamespaceBody(node) {
      var kind = node.kind;
      return kind === 71 || kind === 234;
    }
    ts.isJSDocNamespaceBody = isJSDocNamespaceBody;
    function isNamedImportBindings(node) {
      var kind = node.kind;
      return kind === 242 || kind === 241;
    }
    ...
    function isDeclarationKind(kind) {
      return kind === 188 || kind === 177 || kind === 230 || kind === 200 || kind === 153 || kind === 233 || kind === 268 || kind === 247 || kind === 229 || kind === 187 || kind === 154 || kind === 240 || kind === 238 || kind === 243 || kind === 231 || kind === 257 || kind === 152 || kind === 151 || kind === 234 || kind === 237 || kind === 241 || kind === 147 || kind === 265 || kind === 150 || kind === 149 || kind === 155 || kind === 266 || kind === 232 || kind === 146 || kind === 227 || kind === 288;
    }
    function isDeclarationStatementKind(kind) {
      return kind === 229 || kind === 248 || kind === 230 || kind === 231 || kind === 232 || kind === 233 || kind === 234 || kind === 239 || kind === 238 || kind === 245 || kind === 244 || kind === 237;
    }
    function isStatementKindButNotDeclarationKind(kind) {
      return kind === 219 || kind === 218 || kind === 226 || kind === 213 || kind === 211 || kind === 210 || kind === 216 || kind === 217 || kind === 215 || kind === 212 || kind === 223 || kind === 220 || kind === 222 || kind === 224 || kind === 225 || kind === 209 || kind === 214 || kind === 221 || kind === 291 || kind === 295 || kind === 294;
    }
    function isDeclaration(node) {
      if (node.kind === 146) {
        return node.parent.kind !== 287 || ts.isInJavaScriptFile(node);
      }
    ...
    function isBlockStatement(node) {
      ...
      if (node.parent !== undefined) {
        if (node.parent.kind === 225 || node.parent.kind === 264) {
          return false;
        }
      }
    ...
    function isModuleReference(node) {
      var kind = node.kind;
      return kind === 249 || kind === 144 || kind === 71;
    }
    ts.isModuleReference = isModuleReference;
    function isJsxTagNameExpression(node) {
      var kind = node.kind;
      return kind === 99 || kind === 71 || kind === 180;
    }
    ts.isJsxTagNameExpression = isJsxTagNameExpression;
    function isJsxChild(node) {
      var kind = node.kind;
      return kind === 250 || kind === 260 || kind === 251 || kind === 10 || kind === 254;
    }
    ts.isJsxChild = isJsxChild;
    function isJsxAttributeLike(node) {
      var kind = node.kind;
      return kind === 257 || kind === 259;
    }
    ts.isJsxAttributeLike = isJsxAttributeLike;
    function isStringLiteralOrJsxExpression(node) {
      var kind = node.kind;
      return kind === 9 || kind === 260;
    }
    ts.isStringLiteralOrJsxExpression = isStringLiteralOrJsxExpression;
    function isJsxOpeningLikeElement(node) {
      var kind = node.kind;
      return kind === 252 || kind === 251;
    }
    ts.isJsxOpeningLikeElement = isJsxOpeningLikeElement;
    function isCaseOrDefaultClause(node) {
      var kind = node.kind;
      return kind === 261 || kind === 262;
    }
    ts.isCaseOrDefaultClause = isCaseOrDefaultClause;
    function isJSDocNode(node) {
      return node.kind >= 271 && node.kind <= 289;
    }
    ts.isJSDocNode = isJSDocNode;
    function isJSDocCommentContainingNode(node) {
      return node.kind === 279 || isJSDocTag(node);
    }
    ts.isJSDocCommentContainingNode = isJSDocCommentContainingNode;
    function isJSDocTag(node) {
      return node.kind >= 281 && node.kind <= 289;
    }
  ...
  (function(ts) {
    ...
    function forEachChild(node, cbNode, cbNodes) {
      ...
      switch (node.kind) {
        ...
        case 288:
          if (node.typeExpression && node.typeExpression.kind === 271) {
    ...
    (function(Parser) {
      ...
      function createSourceFile(fileName, languageVersion, scriptKind, isDeclarationFile) {
        var sourceFile = new SourceFileConstructor(269, 0, sourceText.length);
        nodeCount++;
      ...
      function createNode(kind, pos) {
        ...
        return ts.isNodeKind(kind) || kind === 0 ? new NodeConstructor(kind, p, p) : kind === 71 ? new IdentifierConstructor(kind, p, p) : new TokenConstructor(kind, p, p);
      }
      ...
      function createQualifiedName(entity, name) {
        var node = createNode(144, entity.pos);
        node.left = entity;
        node.right = name;
        return finishNode(node);
      }
      ...
      function parseThisTypePredicate(lhs) {
        nextToken();
        var node = createNode(159, lhs.pos);
      ...
      function parseJSDocFunctionType() {
        if (lookAhead(nextTokenIsOpenParen)) {
          var result = createNodeWithJSDoc(277);
          nextToken();
      ...
      function parseParameter() {
        var node = createNodeWithJSDoc(147);
        if (token() === 99) {
      ...
      function parseLiteralTypeNode(negative) {
        ...
        if (negative) {
          unaryMinusExpression = createNode(193);
      ...
      function parseNonArrayType() {
        switch (token()) {
          ...
          case 51:
            return parseJSDocNodeWithType(275);
          case 13:
          case 9:
          case 8:
          case 101:
        ...
        while (!scanner.hasPrecedingLineBreak()) {
          switch (token()) {
            case 58:
              ...
              type = createJSDocPostfixType(276, type);
              break;
            case 51:
            ...
            case 21:
              parseExpected(21);
              if (isStartOfType()) {
                var node = createNode(172, type.pos);
                node.objectType = type;
                node.indexType = parseType();
                parseExpected(22);
                type = finishNode(node);
              } else {
                var node = createNode(165, type.pos);
      ...
      function parseIntersectionTypeOrHigher() {
        return parseUnionOrIntersectionType(168, parseTypeOperatorOrHigher, 48);
      }
      function parseUnionTypeOrHigher() {
        return parseUnionOrIntersectionType(167, parseIntersectionTypeOrHigher, 49);
      }
      ...
      function parseTypeOrTypePredicate() {
        ...
        if (typePredicateVariable) {
          var node = createNode(159, typePredicateVariable.pos);
      ...
      function parseSimpleArrowFunctionExpression(identifier, asyncModifier) {
        ...
        var node;
        if (asyncModifier) {
          node = createNode(188, asyncModifier.pos);
          node.modifiers = asyncModifier;
        } else {
          node = createNode(188, identifier.pos);
        }
        var parameter = createNode(147, identifier.pos);
      ...
      function parseParenthesizedArrowFunctionExpressionHead(allowAmbiguity) {
        var node = createNodeWithJSDoc(188);
      ...
      function parseConditionalExpressionRest(leftOperand) {
        ...
        var node = createNode(196, leftOperand.pos);
        node.condition = leftOperand;
        node.questionToken = questionToken;
        node.whenTrue = doOutsideOfContext(disallowInAndDecoratorContext, parseAssignmentExpressionOrHigher);
        node.colonToken = parseExpectedToken(56, false, ts.Diagnostics._0_expected, ts.tokenToString(56));
        node.whenFalse = ts.nodeIsPresent(node.colonToken) ? parseAssignmentExpressionOrHigher() : createMissingNode(71, false, ts.Diagnostics._0_expected, ts.tokenToString(56));
        return finishNode(node);
      }
      ...
      function makeAsExpression(left, right) {
        var node = createNode(203, left.pos);
      ...
      function parseUnaryExpressionOrHigher() {
        ...
        if (token() === 40) {
          ...
          if (simpleUnaryExpression.kind === 185) {
      ...
      function parseJsxElementOrSelfClosingElementOrFragment(inExpressionContext) {
        ...
        var result;
        if (opening.kind === 252) {
          var node = createNode(250, opening.pos);
          ...
          ts.Debug.assert(opening.kind === 251);
          result = opening;
        }
        if (inExpressionContext && token() === 27) {
          ...
          if (invalidElement) {
            ...
            var badNode = createNode(195, result.pos);
      ...
      function parseJsxAttributes() {
        var jsxAttributes = createNode(258);
      ...
      function parseJsxOpeningOrSelfClosingElementOrOpeningFragment(inExpressionContext) {
        ...
        if (token() === 29) {
          parseExpected(29);
          var node_1 = createNode(255, fullStart);
      ...
      function parseJsxElementName() {
        ...
        while (parseOptional(23)) {
          var propertyAccess = createNode(180, expression.pos);
      ...
      function parseMemberExpressionRest(expression) {
        while (true) {
          ...
          if (token() === 51 && !scanner.hasPrecedingLineBreak()) {
            nextToken();
            var nonNullExpression = createNode(204, expression.pos);
            nonNullExpression.expression = expression;
            expression = finishNode(nonNullExpression);
            continue;
          }
          if (!inDecoratorContext() && parseOptional(21)) {
            var indexedAccess = createNode(181, expression.pos);
          ...
          if (token() === 13 || token() === 14) {
            var tagExpression = createNode(184, expression.pos);
      ...
      function parseCallExpressionRest(expression) {
        while (true) {
          ...
          if (token() === 27) {
            ...
            var callExpr = createNode(182, expression.pos);
      ...
      function parseParenthesizedExpression() {
        var node = createNodeWithJSDoc(186);
        parseExpected(19);
        node.expression = allowInAnd(parseExpression);
        parseExpected(20);
        return finishNode(node);
      ...
      function parseArgumentOrArrayLiteralElement() {
        return token() === 24 ? parseSpreadElement() : token() === 26 ? createNode(201) : parseAssignmentExpressionOrHigher();
      }
      ...
      function parseObjectLiteralElement() {
        ...
        if (parseContextualModifier(125)) {
          return parseAccessorDeclaration(node, 154);
        }
      ...
      function parseFunctionExpression() {
        ...
        var node = createNodeWithJSDoc(187);
        node.modifiers = parseModifiers();
        parseExpected(89);
        node.asteriskToken = parseOptionalToken(39);
      ...
      function parseForOrForInOrForOfStatement() {
        ...
        if (awaitToken ? parseExpected(143) : parseOptional(143)) {
          var forOfStatement = createNode(217, pos);
          ...
          var forInStatement = createNode(216, pos);
          forInStatement.initializer = initializer;
          forInStatement.expression = allowInAnd(parseExpression);
          parseExpected(20);
          forOrForInOrForOfStatement = forInStatement;
        } else {
          var forStatement = createNode(215, pos);
      ...
      function parseStatement() {
        switch (token()) {
          ...
          case 104:
            return parseVariableStatement(createNodeWithJSDoc(227));
          case 110:
            if (isLetDeclaration()) {
              return parseVariableStatement(createNodeWithJSDoc(227));
            }
            break;
          case 89:
            return parseFunctionDeclaration(createNodeWithJSDoc(229));
          case 75:
            return parseClassDeclaration(createNodeWithJSDoc(230));
          case 90:
          ...
          case 72:
            return parseBreakOrContinueStatement(219);
          case 96:
      ...
      function parseDeclaration() {
        ...
        if (ts.some(node.modifiers, isDeclareModifier)) {
          ...
          return doInsideOfContext(2097152, function() {
        ...
        switch (token()) {
          ...
          default:
            if (node.decorators || node.modifiers) {
              var missing = createMissingNode(248, true, ts.Diagnostics.Declaration_expected);
      ...
      function parseVariableDeclaration(allowExclamation) {
        ...
        node.name = parseIdentifierOrPattern();
        if (allowExclamation && node.name.kind === 71 && token() === 51 && !scanner.hasPrecedingLineBreak()) {
      ...
      function parsePropertyDeclaration(node) {
        ...
        node.type = parseTypeAnnotation();
        node.initializer = ts.hasModifier(node, 32) ? allowInAnd(parseInitializer) : doOutsideOfContext(4096 | 2048, parseInitializer);
        parseSemicolon();
        return finishNode(node);
      }
      ...
      function parseDecorators() {
        ...
        while (true) {
          ...
          var decorator = createNode(148, decoratorStart);
      ...
      function parseClassExpression() {
        return parseClassDeclarationOrExpression(createNodeWithJSDoc(0), 200);
      }
      function parseClassDeclaration(node) {
        return parseClassDeclarationOrExpression(node, 230);
      }
      ...
      function parseEnumMember() {
        var node = createNodeWithJSDoc(268);
        node.name = parsePropertyName();
      ...
      function parseModuleOrNamespaceDeclaration(node, flags) {
        ...
        node.flags |= flags;
        node.name = parseIdentifier();
        node.body = parseOptional(23) ? parseModuleOrNamespaceDeclaration(createNode(0), 4 | namespaceFlag) : parseModuleBlock();
        return finishNode(node);
      }
      ...
      function parseImportClause(identifier, fullStart) {
        var importClause = createNode(240, fullStart);
        if (identifier) {
          importClause.name = identifier;
        }
        if (!importClause.name || parseOptional(26)) {
          importClause.namedBindings = token() === 39 ? parseNamespaceImport() : parseNamedImportsOrExports(242);
        }
      ...
      function parseNamespaceImport() {
        var namespaceImport = createNode(241);
        parseExpected(39);
        parseExpected(118);
      ...
      function parseNamedImportsOrExports(kind) {
        var node = createNode(kind);
        node.elements = parseBracketedList(22, kind === 242 ? parseImportSpecifier : parseExportSpecifier, 17, 18);
        return finishNode(node);
      }
      ...
      function parseImportOrExportSpecifier(kind) {
        ...
        if (kind === 243 && checkIdentifierIsKeyword) {
      ...
      function parseExportDeclaration(node) {
        ...
        if (parseOptional(39)) {
          ...
          node.exportClause = parseNamedImportsOrExports(246);
          if (token() === 141 || (token() === 9 && !scanner.hasPrecedingLineBreak())) {
            parseExpected(141);
            node.moduleSpecifier = parseModuleSpecifier();
          }
        }
        ...
        while (true) {
          ...
          if (referencePathMatchResult) {
            ...
            if (checkJsDirectiveMatchResult) {
              checkJsDirective = {
                enabled: ts.equateStringsCaseInsensitive(checkJsDirectiveMatchResult[1], "@ts-check"),
                end: range.end,
                pos: range.pos
              };
            }
      ...
      function setExternalModuleIndicator(sourceFile) {
        sourceFile.externalModuleIndicator = ts.forEach(sourceFile.statements, function(node) {
          return ts.hasModifier(node, 1) || node.kind === 238 && node.moduleReference.kind === 249 || node.kind === 239 || node.kind === 244 || node.kind === 245 ? node : undefined;
        });
      }
      var JSDocParser;
      (function(JSDocParser) {
        ...
        function parseJSDocTypeExpression(mayOmitBraces) {
          var result = createNode(271, scanner.getTokenPos());
        ...
        function parseJSDocCommentWorker(start, length) {
          ...
          function parseParameterOrPropertyTag(atToken, tagName, target) {
            ...
            var result = target === 1 ? createNode(284, atToken.pos) : createNode(289, atToken.pos);
          ...
          function parseNestedTypeLiteral(typeExpression, name) {
            if (typeExpression && isObjectOrObjectArrayTypeReference(typeExpression.type)) {
              var typeLiteralExpression = createNode(271, scanner.getTokenPos());
              var child = void 0;
              ...
              if (children) {
                ...
                if (typeExpression.type.kind === 165) {
          ...
          function parsePropertyAccessEntityNameExpression() {
            ...
            while (parseOptional(23)) {
              var prop = createNode(180, node.pos);
          ...
          function parseClassTag(atToken, tagName) {
            var tag = createNode(283, atToken.pos);
            tag.atToken = atToken;
            tag.tagName = tagName;
          ...
          function parseTypedefTag(atToken, tagName) {
            var typeExpression = tryParseTypeExpression();
            skipWhitespace();
            var typedefTag = createNode(288, atToken.pos);
            ...
            if (!typeExpression || isObjectOrObjectArrayTypeReference(typeExpression.type)) {
              ...
              while (child = tryParse(function() {
                ...
                if (!jsdocTypeLiteral) {
                  jsdocTypeLiteral = createNode(280, start_3);
                }
                if (child.kind === 286) {
                  if (childTypeTag) {
                    break;
              ...
              if (jsdocTypeLiteral) {
                if (typeExpression && typeExpression.type.kind === 165) {
                  jsdocTypeLiteral.isArrayType = true;
                }
            ...
            function parseJSDocTypeNameWithNamespace(flags) {
              ...
              if (typeNameOrNamespaceName && parseOptional(23)) {
                var jsDocNamespaceNode = createNode(234, pos);
              ...
              switch (nextJSDocToken()) {
                case 57:
                  if (canParseTag) {
                    ...
                    if (child && child.kind === 284 && (ts.isIdentifier(child.name) || !escapedTextsEqual(name, child.name.left))) {
                      return false;
                    }
                    return child;
                  }
          ...
          function parseTemplateTag(atToken, tagName) {
            ...
            while (true) {
              var typeParameter = createNode(146);
  ...
  (function(ts) {
    ...
    function parseConfig(json, sourceFile, host, basePath, configFileName, resolutionStack, errors) {
      ...
      var ownConfig = json ? parseOwnConfigOfJson(json, host, basePath, configFileName, errors) : parseOwnConfigOfJsonSourceFile(sourceFile, host, basePath, configFileName, errors);
    ...
    (function(JsTyping) {
      ...
      function discoverTypings(host, log, fileNames, projectRootPath, safeList, packageNameToTypingLocation, typeAcquisition, unresolvedImports) {
        ...
        if (unresolvedImports) {
          ...
          }), ts.equateStringsCaseSensitive, ts.compareStringsCaseSensitive);
        ...
        function getTypingNamesFromPackagesFolder(packagesFolderPath, filesToWatch) {
          ...
          for (var _i = 0,
            ...
            var result_1 = ts.readConfigFile(normalizedFileName, function(path) {
              return host.readFile(path);
            });
  ...
  (function(ts) {
    ...
    function getUnmangledNameForScopedPackage(typesPackageName) {
      return ts.stringContains(typesPackageName, mangledScopedPackageSeparator) ? "@" + typesPackageName.replace(mangledScopedPackageSeparator, ts.directorySeparator) : typesPackageName;
    }
        ...
        var TypingsInstaller = (function() {
          ...
          TypingsInstaller.prototype.filterTypings = function(typingsToInstall) {
            ...
            return typingsToInstall.filter(function(typing) {
              ...
              if (_this.packageNameToTypingLocation.get(typing)) {
                if (_this.log.isEnabled())
                  _this.log.writeLine("'" + typing + "' already has a typing - skipping...");
                return false;
              }
                ...
                case "installPackage":
                  {
                    ...
                    if (cwd) {
                      _this.installWorker(-1, [packageName_1], cwd, function(success) {
                        ...
                        var response = {
                          kind: server.ActionPackageInstalled,
